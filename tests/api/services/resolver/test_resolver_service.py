...
def test_find_project_and_scan_existing(resolver_service, mock_projects_client, mock_scans_client):
    mock_projects_client.list_projects.return_value = [
        {"project_name": "TestProject", "project_code": "PROJ123"}
    ]
...
def test_find_scan_globally_multiple_results(resolver_service, mock_scans_client):
    mock_scans_client.list_scans.return_value = [
        _scan_row(
            name="DupeScan",
            code="SCAN111",
...
def test_create_scan_success(resolver_service, mock_projects_client, mock_scans_client):
...
def test_find_or_create_both_exist(resolver_service, mock_projects_client, mock_scans_client):
...
def test_find_or_create_create_project(resolver_service, mock_projects_client, mock_scans_client):
    mock_projects_client.list_projects.return_value = []
...
def test_find_or_create_create_scan(resolver_service, mock_projects_client, mock_scans_client):
    mock_projects_client.list_projects.return_value = [
        {"project_name": "TestProject", "project_code": "PROJ123"}
    ]
    mock_projects_client.get_all_scans.side_effect = [
    ...
    result = resolver_service.find_or_create("TestProject", "NewScan", scan_data={})
