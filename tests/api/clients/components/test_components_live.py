...
class TestComponentsLiveReadOnly:
    def test_list_components(self, workbench_client, workbench_version):
    ...
    def test_list_by_usage(self, workbench_client, workbench_version):
        data = workbench_client.components.list_by_usage(page=1, records_per_page=5)
    ...
    def test_get_information_from_list(self, workbench_client, workbench_version):
        listed = workbench_client.components.list_components(records_per_page=1, page=1)
        ...
        data = workbench_client.components.get_information(name, version)
        assert_data_contract(
    ...
    def test_list_components_count_only(self, workbench_client, workbench_version):
        response = workbench_client.components._api._send_request(
            {
                "group": "components",
                "action": "list_components",
    ...
    def test_get_usage_for_listed_component(self, workbench_client, workbench_version):
        listed = workbench_client.components.list_components(records_per_page=5, page=1)
        if not isinstance(listed, list) or not listed:
    ...
    def test_get_usage_count_if_component_exists(self, workbench_client, workbench_version):
        listed = workbench_client.components.list_components(records_per_page=1, page=1)
        if not isinstance(listed, list) or not listed:
...
class TestComponentsLiveMutations:
    def test_create_and_delete_component(
        ...
        try:
            deleted = workbench_client.components.delete(unique_component_name, version)
