...
# Tests end-to-end workflow: import-sbom → show-results → evaluate-gates → download-reports
...
show_progress() {
    ...
    local steps=("import-sbom" "results" "gates" "reports-project" "reports-scan")
    local progress=""
    ...
    progress=${progress% -> }
    echo -e "${BLUE}[IMPORT-SBOM]${NC} Progress: $progress"
}
...
if [ -f "$FIXTURES_DIR/cyclonedx-bom.json" ]; then
    run_test "Step 1: Import-SBOM" "import-sbom" \
    ...
    echo -e "${RED}ERROR: cyclonedx-bom.json not found at $FIXTURES_DIR${NC}"
