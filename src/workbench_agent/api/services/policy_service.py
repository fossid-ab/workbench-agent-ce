...
class PolicyService:
    ...
    def get_policy_warnings(self, scan_code: str) -> Dict[str, Any]:
        ...
        """
        logger.debug("Fetching policy warnings counter for scan '%s'", scan_code)
        warnings = self._scans.get_policy_warnings_counter(scan_code)
        logger.debug("Retrieved policy warnings counter")
        return warnings

    def get_scan_identification_policy_warnings_info(self, scan_code: str) -> Dict[str, Any]:
        """
        Policy rules and per-rule ``findings`` from scan file identifications.

        Returns ``policy_warnings_list`` (may be empty).
        """
        return self._scans.get_policy_warnings_info(scan_code, warning_type="identifications")

    def get_scan_dependency_policy_warnings_info(self, scan_code: str) -> Dict[str, Any]:
        """Policy rules and findings from dependency analysis only."""
        return self._scans.get_policy_warnings_info(scan_code, warning_type="dependencies")

    def get_scan_policy_warnings_info_all(self, scan_code: str) -> Dict[str, Any]:
        """Policy rules and findings from identifications and dependencies."""
        return self._scans.get_policy_warnings_info(scan_code, warning_type="all")

    # ===== PROJECT-LEVEL POLICY OPERATIONS =====

    def get_project_identification_policy_warnings(self, project_code: str) -> Dict[str, Any]:
        """
        ...
        """
        return self._projects.get_policy_warnings_info(project_code, warning_type="identifications")

    def get_project_dependency_policy_warnings(self, project_code: str) -> Dict[str, Any]:
        """Project policy warnings from dependency analysis only."""
        return self._projects.get_policy_warnings_info(project_code, warning_type="dependencies")

    def get_project_policy_warnings_all(self, project_code: str) -> Dict[str, Any]:
        """Project policy warnings from identifications and dependencies."""
        return self._projects.get_policy_warnings_info(project_code, warning_type="all")

    # ===== DOWNLOAD PROJECT POLICY JSON =====

    def download_project_policy_json(self, project_code: str) -> Union[List[Any], Dict[str, Any]]:
        """
        ...
        """
        logger.debug("Downloading project policy JSON for '%s'", project_code)
        ...
        if raw is not None:
            try:
                ...
                raise ApiError(f"Invalid project policy JSON for '{project_code}': {exc}") from exc
        ...
        raise ApiError(f"Unexpected project policy response for '{project_code}'")
