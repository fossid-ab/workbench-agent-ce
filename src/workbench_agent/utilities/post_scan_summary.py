...
def print_scan_summary(
    ...
    if scan_operations is None:
        raise ValueError(
            "scan_operations is required when show_summary=True"
        )
    ...
    if kb_scan_performed:
        ...
        if scan_metrics:
            ...
            print(
                f"  - Files with Identifications: {identified_files}"
            )
            ...
            if (
                ...
                print(
                    f"    - Components Identified: {num_components}"
                )
            ...
            if total_files == 0 or (
                isinstance(total_files, str) and total_files == "0"
            ):
                print("\n  Note: There were no files to scan.")
        else:
            print(
                "  - Files Scanned: N/A (could not fetch metrics)"
            )
    ...
    if policy_warnings is not None:
        ...
        if total_warnings > 0:
            print(
                f"    - In Identified Files: {files_with_warnings}"
            )
